...
var gestures = {
        ...
        if (__rotation_single_finger && utils.getFingers(ev) < 2) {
            ...
            if (__rotation_single_start.length < 2) {
                ...
                __rotation_single_start = [{
                        x: docOff.x + el.offsetWidth / 2,
                        y: docOff.y + el.offsetHeight / 2
                    },
                    pos.move[0]
                ];
    ...
    swipe: function(ev) {
        ...
        var position = {
            x: pos.move[0].x - __offset.x,
            y: pos.move[0].y - __offset.y
        };
...
var handlerOriginEvent = function(ev) {
    ...
    switch (ev.type) {
        ...
        case 'mousedown':
            ...
            __offset = {
                top: box.y + (window.pageYOffset || docEl.scrollTop) - (docEl.clientTop || 0),
                left: box.x + (window.pageXOffset || docEl.scrollLeft) - (docEl.clientLeft || 0)
            };
